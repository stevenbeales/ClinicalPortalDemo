	...
	$.jstree.plugin("crrm", { 
		...
		_fn : {
			_show_input : function (obj, callback) {
				...
				this.set_text(obj, "");
				h2.autoGrowInput({	//similute highlighted effect
					comfortZone: 20
				}).select();
			},
			rename : function (obj) {
			...
			_setupSearchField : function() {
				...
				searchField
					...
					.closest(".inforTriggerField").css("margin","2px 0 2px 8px").find(".inforTriggerButton").click(function() {
						...
						if (term==field.attr("placeholder"))
							term="";
	...
	$.jstree.plugin("contextmenu", {
		...
		_fn : {
			show_contextmenu : function (e) {
				...
				for (var key in s.items) {
					if (s.items[key]==false || s.items[key].label==undefined)
						continue;
						
					var a = $('<a href="#'+key+'">'+s.items[key].label+'</a>');
					menuHtml.append($('<li></li>').append(a));
					
					//Add Submenu
					if (s.items[key].submenu) {
						var ul = $('<ul class="inforContextMenu"></ul>');
						a.after(ul);
						for (var sub in s.items[key].submenu) {
							var a = $('<a></a>').text(s.items[key].submenu[sub].label)
												.attr("href","#"+key+"_"+sub);
				...
				obj.inforContextMenu({
					...
					if (id.indexOf("_")>-1)
						func = s.items[id.substr(0,id.indexOf("_"))].submenu[id.substr(id.indexOf("_")+1)].action;
					else
					    func = s.items[id].action;
